...
void PROPAGATE::euler(const PARAMS& p){
  double tavel = avel;
  avel=avel-(p.gbl*ang+p.kappa*avel-p.F_D*sin(2.0*pi*time/p.T_D))*dt; 
  ang=ang+tavel*dt;  
  time+=dt;
};
...
void PROPAGATE::runge_kutta(const PARAMS& p){
  ...
  double avelprime=avel-(p.gbl*ang+p.kappa*avel-p.F_D*sin(2.0*pi*time/p.T_D))*dt/2.0;
  //euler half step for ang
  double angprime = ang+avel*dt/2.0;
  avel=avel-(p.gbl*angprime+p.kappa*avelprime-p.F_D*sin(2.0*pi*tprime/p.T_D))*dt;
